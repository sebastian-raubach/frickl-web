      ...
      <div v-if="hasXAgo">
        ...
        <b-row class="mb-3 pb-3">
          <b-col cols=12 md=4 lg=3 v-if="xAgo.one && xAgo.one.length > 0">
            <AlbumNode :album="xAgo.one[0]" title="One year ago" to="/xago/1" :albumCount="xAgo.one.length" :imageCount="xAgoImageCount.one" :imageCountPublic="xAgoImageCountPublic.one" />
          </b-col>
          <b-col cols=12 md=4 lg=3 v-if="xAgo.two && xAgo.two.length > 0">
            <AlbumNode :album="xAgo.two[0]" title="Two years ago" to="/xago/2" :albumCount="xAgo.two.length" :imageCount="xAgoImageCount.two" :imageCountPublic="xAgoImageCountPublic.two" />
          </b-col>
          <b-col cols=12 md=4 lg=3 v-if="xAgo.three && xAgo.three.length > 0">
            <AlbumNode :album="xAgo.three[0]" title="Three years ago" to="/xago/3" :albumCount="xAgo.three.length" :imageCount="xAgoImageCount.three" :imageCountPublic="xAgoImageCountPublic.three" />
          </b-col>
        </b-row>
      </div>
...
export default {
  ...
  computed: {
    ...
    xAgoImageCount: function () {
      return {
        one: this.xAgo.one ? this.xAgo.one.map(a => a.imageCount).reduce((a, b) => a + b, 0) : null,
        two: this.xAgo.two ? this.xAgo.two.map(a => a.imageCount).reduce((a, b) => a + b, 0) : null,
        three: this.xAgo.three ? this.xAgo.three.map(a => a.imageCount).reduce((a, b) => a + b, 0) : null
      }
    },
    xAgoImageCountPublic: function () {
      return {
        one: this.xAgo.one ? this.xAgo.one.map(a => a.imageCountPublic).reduce((a, b) => a + b, 0) : null,
        two: this.xAgo.two ? this.xAgo.two.map(a => a.imageCountPublic).reduce((a, b) => a + b, 0) : null,
        three: this.xAgo.three ? this.xAgo.three.map(a => a.imageCountPublic).reduce((a, b) => a + b, 0) : null
      }
    }
  },
  components: {
