  ...
  <v-container fluid>
    ...
    <v-sheet border>
      ...
      >
        ...
        <template #top>
          <v-toolbar flat class="justify-space-between">
            <v-toolbar-title class="flex-unset">
              <v-icon color="medium-emphasis" icon="mdi-account-multiple" start />
        ...
        <template v-for="permission in allPermissions" :key="`${permission.code}`" #[`item.${permission.name}`]="{ item }">
          <v-checkbox-btn disabled :model-value="hasPermission(permission, item.permissions)" />
        </template>

        <template #item.username="{ item, value }">
          <v-chip class="w-100" :text="value" border="thin opacity-25" :prepend-icon="hasPermission(allPermissions.find(p => p.name === 'IS_ADMIN'), item.permissions)? 'mdi-account-star' : 'mdi-account'" label>
            <template #prepend>
    ...
    <v-dialog v-model="dialog" max-width="500">
      <v-card :title="isEdit ? $t('modalTitleUserEdit') : $t('modalTitleUserAdd')">
        <template #text>
          ...
          <v-checkbox v-for="permission in allPermissions" :key="`permission-${permission.code}`" :model-value="hasPermission(permission, record.permissions)" hide-details :label="permission.name" @update:model-value="togglePermission(record, permission)" />
        </template>
        ...
        <v-card-actions class="bg-surface-light">
          ...
          <v-btn :text="$t('buttonSave')" @click="save" />
        </v-card-actions>
      </v-card>
    </v-dialog>
  ...
  export default {
    ...
    computed: {
      ...
      viewTypeOptions: function () {
        return this.viewTypeTexts
          ? Object.keys(this.viewTypeTexts).map(k => {
            return {
              value: k,
      ...
      allPermissions: function () {
        return this.storeToken && this.storeToken.allPermissions ? this.storeToken.allPermissions : []
      },
      columns: function () {
        return [{
    ...
    methods: {
      ...
      save: function () {
        if (this.isEdit) {
          ...
          if (!this.record.username || this.record.username.trim().length === 0) {
            this.formState.username = false
          }
          if (!this.record.password || this.record.password.trim().length === 0) {
            this.formState.password = false
          }
          if (Object.values(this.formState).includes(false)) {
            return
          }
